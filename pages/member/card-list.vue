			...
			<view class="card-list" v-for="(item, index) in options" :key="index" :class="{ up: selectKey == index }">
				<view class="title" @click="selListItem(index, item.id, item)">
					<view class="box">
						...
						<view class="ft">
							<view class="block">
								<text class="word" style="color: #000000;">日期</text>
							</view>
							<view class="group">
								<text class="word">{{ item.exp_or_|| '未设置' }}</text>
							</view>
							<view class="block">
								<text class="word" style="color: #000000;">CVV</text>
							</view>
							<view class="group">
								<text class="word">{{ item.cvv2_or_ || '未设置'}}</text>
							</view>
						</view>
						<view class="ft" v-if="0">
							<view class="block">
	...
	export default {
		...
		methods: {
			...
			selListItem(index, aaa, item) {
				...
				// 	this.selectKey = index;
				// }
				let bgColor = ['#f5ad1b', '#5f89ce', '#94bf45', '#da8ec5', '#78bfc2', '#bec278', '#f5ad1c', '#5f89ca', '#94bf4a',
					'#da8eca', '#78bfca', '#bec27a', '#f5ad10', '#5f89cc', '#94bf4c', '#da8ecc', '#78bfcc', '#bec27c'
				]
				uni.navigateTo({
