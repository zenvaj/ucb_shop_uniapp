<template>
	<view class="container">
		<list-cell title="银行名称" type="text" placeholder="" v-model="userName"></list-cell>

		<list-cell title="银行账户" type="number" placeholder="" v-model="mobile"></list-cell>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.$Request.postP('/bank/detail', {}).then(res => {
				console.log(res);
				if(res.status != 10000){
					uni.showModal({
						content:"未设置储蓄卡信息",
				...
				this.userName = res.data.bankname
				this.mobile = res.data.banknumber
			});
