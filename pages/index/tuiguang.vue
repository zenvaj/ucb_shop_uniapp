<template style="background: white">
	<view>
		<view style="background: white;z-index: 999">
			...
			>
				<view class="cu-item" :class="indexs == TabCurs ? 'text-green cur' : ''" v-for="(item, indexs) in tab" :key="indexs" @tap="searchSelect" :data-id="indexs">
		...
		<view class="index-content" style="padding-top: 80upx;">
			<view class="index-coupon has-bg-white has-pd-10 top_30">
				<view class="goods-list" v-if="couponlist.length > 0">
					<orange-goods-card-home v-for="(item,index) in couponlist" :index="index%2" :logo="logo" :is-invitation="isInvitation"
					 :tkmoney='item.tkmoney' :shopname='item.shopname' :isEnable='isEnable' :tkmoneys='item.tkmoneys' :itemid="item.itemid" :itempic="item.itempic+'_310x310.jpg'"
