<template>
	<view>
		...
		<view style="color:#cccccc;width: 90%;margin: auto;padding:0 20upx;font-size: 30upx;border-bottom: 1upx solid rgba(0,0,0,0.3)"
		 class="bg-white" v-if="selectedDate.length!==0">
			<text>日期：</text>
			...
			this.$Request
				...
				.then(res => {
					...
					if (res.status === 10000) {
						...
						if (mm >= 11) {
							this.endDate = (yy + 1) + "-01-" + this.dayjob_or_;
							if (this.dayjob_or_ < this.daymark_or_) {
								this.endDate = (yy + 1) + "-02-" + this.dayjob_or_;
							}
						} else {
							this.endDate = yy + "-" + (mm + 1) + "-" + this.dayjob_or_;
							if (this.dayjob_or_ < this.daymark_or_) {
								this.endDate = yy + "-" + (mm + 2) + "-" + this.dayjob_or_;
							}
						}
