<template>
	<view class="s-page-wrapper" v-if="showPage">
		<view class="index-goods" v-if="goods && goods.imageInfo">
			...
			<view
				style="background: rgba(0, 0, 0, 0.7);padding: 8px;display: flex;font-size: 14px;font-weight: bold"
			...
			<view class="goods_info" style="padding-top: 12upx;padding-bottom: 20upx">
				<view class="coupon-price s-row">
					...
					<view class="volume" style="padding-top:14upx;font-size: 14px;width: 30%">{{ goods.inOrderCount30Days }}人付款</view>
				</view>
				...
				<view
					v-if="goods.shopInfo && goods.shopInfo.shopName"
					style="float: right;height: 40upx;line-height: 40upx;margin-top: -16upx;padding-right: 16upx;text-align: right;display: flex;"
				>
...
export default {
	...
	methods: {
			...
			this.$Request.getT('/jd/goods/' + this.itemid).then(res => {
				if (res.code === 200) {
					...
					if (grade) {
						...
						that.money = (item.commissionInfo.couponCommission * parseFloat(this.$queue.minMoney())).toFixed(2);
					}
					item.tkmoney = that.money;
					item.tkmoneys = (item.commissionInfo.couponCommission * parseFloat(this.$queue.maxMoney())).toFixed(2);
					item.itemprice = that.money;
					that.shengji = (item.commissionInfo.couponCommission * parseFloat(this.$queue.maxMoney())).toFixed(2);
					...
					item.itemendprice = (item.priceInfo.price - item.couponInfo.couponList[0].discount).toFixed(2);
					that.grade = grade;
		...
		copyTklWenAns: function() {
			uni.setClipboardData({
				data:
					...
					'元\n【券后价】' +
					this.goods.priceInfo.price -
					this.goods.couponInfo.couponList[0].discount +
					'元' +
					'+\n下单链接：' +
					this.erweima +
					'\n',
		...
		getJDUrl(type) {
			...
			this.$Request.getT('/jd/goods/couponUrl?positionId=' + jdpid + '&materialId=' + url + '&couponUrl=' + coupons).then(res => {
				if (res.data) {
